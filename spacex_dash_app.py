...
def get_pie_chart(input_site_dropdown):
        ...
        if input_site_dropdown == 'ALL':
            ...
            filtered_df = spacex_df[spacex_df['Launch Site'] == input_site_dropdown]
            fig = px.pie(filtered_df, 
                        names='class', 
                        title=f'Success and Failure Launches for {input_site_dropdown}')
            return fig
...
def get_scatter_plot(input_site_dropdown, input_payload_slider):
        filtered_df = spacex_df[
            (spacex_df['Payload Mass (kg)'] >= input_payload_slider[0]) &
            (spacex_df['Payload Mass (kg)'] <= input_payload_slider[1])
        ]
        if input_site_dropdown == 'ALL':
            fig = px.scatter(filtered_df,
                            ...
                            title= 'Payload Mass and Launch Outcome')
            return fig
        else:
            filtered_df = filtered_df[filtered_df['Launch Site'] == input_site_dropdown]
            fig = px.scatter(filtered_df, 
                            x='Payload Mass (kg)',
                            y='class',
                            color='Booster Version Category',
                            labels={'Payload Mass (kg)': 'Payload Mass (kg)', 'class': 'Launch Success'},
                            title=f'Payload Mass and Launch Outcome for {input_site_dropdown}')
            return fig
